...
class DML(LinearModelFinalCateEstimatorMixin, _BaseDML):
    ...
    def refit(self, model_final, featurizer=None, fit_cate_intercept=True):
        ...
        self : DML
            This instance
        """
        if self.linear_first_stages:
            warn("Using original featurizer because first stage Y results depend on it "
                 "when linear_first_stages is true")
            featurizer = self.original_featurizer
        self.bias_part_of_coef = fit_cate_intercept
        self.fit_cate_intercept = fit_cate_intercept
        return super().refit(_FinalWrapper(model_final, fit_cate_intercept, featurizer, False))
...
class LinearDML(StatsModelsCateEstimatorMixin, DML):
    ...
    def refit(featurizer=None, fit_cate_intercept=True):
        """
        Estimate the counterfactual model using a new final model specification but with cached first stage results.
        ...
        self : LinearDML
            This instance
        """
        return super().refit(StatsModelsLinearRegression(fit_intercept=False),
                             featurizer=featurizer, fit_cate_intercept=fit_cate_intercept)
...
class SparseLinearDML(DebiasedLassoCateEstimatorMixin, DML):
    ...
    def refit(self, alpha='auto', max_iter=1000, tol=1e-4, featurizer=None, fit_cate_intercept=True):
        """
        Estimate the counterfactual model using a new final model specification but with cached first stage results.
        ...
        Returns
        -------
        self : SparseLinearDML
            This instance
        """
        return super().refit(MultiOutputDebiasedLasso(alpha=alpha,
                                                      fit_intercept=False,
                                                      max_iter=max_iter,
                                                      tol=tol),
                             featurizer=featurizer, fit_cate_intercept=fit_cate_intercept)
...
class ForestDML(ForestModelFinalCateEstimatorMixin, NonParamDML):
    ...
    def refit(self, n_estimators=100, criterion="mse", max_depth=None, min_samples_split=2, min_samples_leaf=1,
              min_weight_fraction_leaf=0., max_features="auto", max_leaf_nodes=None, min_impurity_decrease=0.,
              subsample_fr='auto', honest=True, n_jobs=None, verbose=0, random_state=None):
        """
        Estimate the counterfactual model using a new final model specification but with cached first stage results.
        ...
        return super().refit(SubsampledHonestForest(n_estimators=n_estimators,
                                                    ...
                                                    n_jobs=n_jobs,
                                                    random_state=random_state,
                                                    verbose=verbose), featurizer=None)
